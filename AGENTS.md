...
- `lib/graph/` builds `.aegis/context/graph.json`; L0 import graph is default, L1 symbols/signatures require `npx aegis-spec graph build --level L1` and optional native parser deps.
- Control-plane gates: build `.aegis/context/policy-index.json` with `npx aegis-spec policy-index build`, then run `npx aegis-spec policy-check --base <ref> --head HEAD`.
...
- Aegis Spec-generated runtime state belongs under `.aegis/`; generated specs default to `_aegis_sdd/`.
- Keeper auto mode is opt-in: `_aegis_sdd/auto-policy.yaml` must enable `auto_resolve`, and live LLM mode needs `ANTHROPIC_API_KEY`; `--dry-run` skips LLM/spec writes.
- `drift-check` is standalone and engine-agnostic; it reads `<output_folder>/drift.md`, defaulting from `.aegis/state.json` then `_aegis_sdd`.
...
- `.aegis/`, `.claude/`, `site/`, `dist/`, and `specs/` are ignored; avoid relying on ignored generated state for committed changes.
