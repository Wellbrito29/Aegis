// Install a git pre-commit hook that runs `reversa policy-check --severity
...
const BEGIN = '# >>> reversa policy-check >>>';
const END = '# <<< reversa policy-check <<<';

const HOOK_BODY = [
  BEGIN,
  '# Installed by `reversa install --git-hooks`. Remove the block between',
  '# the markers (or run `reversa remove-hooks --git`) to disable.',
  'if command -v npx >/dev/null 2>&1; then',
  '  npx --no-install reversa policy-check --base HEAD --head :0 --severity medium || exit 1',
  'fi',
  END,
].join('\n');
