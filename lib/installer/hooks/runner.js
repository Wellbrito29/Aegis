#!/usr/bin/env node
//
// Aegis Spec Keeper hook runner — lean append-only mode (v1.7.0+).
//
// Engine-agnostic. Invoked by hooks installed by `npx aegis add-hooks`.
//
// Phases:
...
//   - post  : append a single JSON line to aegis/runtime/queue/keeper-queue.jsonl
...
//   - Errors logged to aegis/runtime/audit/keeper-errors.log (best-effort).
//
...
function findProjectRoot(startDir) {
  ...
  while (dir !== dirname(dir)) {
    if (existsSync(join(dir, 'aegis', 'config', 'state.json'))) return dir;
    dir = dirname(dir);
  }
  return startDir;
}
...
function logError(projectRoot, message) {
  try {
    const logPath = join(projectRoot, 'aegis', 'runtime', 'audit', 'keeper-errors.log');
...
function appendJsonl(projectRoot, entry) {
      const queuePath = join(projectRoot, 'aegis', 'runtime', 'queue', 'keeper-queue.jsonl');
  ...
  const queuePath = join(projectRoot, 'aegis', 'runtime', 'queue', 'keeper-queue.jsonl');
      if (existsSync(queuePath)) {
        ...
        if (editLines.length > 0) {
          ...
          process.stderr.write(`\n[aegis-keeper] ${editLines.length} edits queued (${editFiles.size} files). Run /aegis-keeper after to update specs.\n\n`);
        }
      }
    } catch { /* swallow */ }
...
async function refreshGraph(projectRoot, dirtyFiles) {
  const graphPath = join(projectRoot, 'aegis', 'runtime', 'context', 'graph.json');
