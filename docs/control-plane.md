...
Reversa 2.0 ships a control plane around AI-generated code. Three pillars
work together:

| Pillar | What it does | Where it lives |
|---|---|---|
| **Reversa** | Spec authority — features, contracts, invariants, ADRs | `_reversa_sdd/` |
| **Keeper** | Drift gate — keeps spec and code in sync, optionally via LLM | `agents/reversa-keeper/` + `lib/auto/` |
...
| **Audit log** | Every decision | Persist who/what/why under `.reversa/audit/` |
...
`_reversa_sdd/auto-policy.yaml`. See `docs/keeper-auto.md`.

## Files Reversa adds to your repo

| Path | Purpose |
|---|---|
| `_reversa_sdd/sdd/*.md` | Specs with frontmatter (`contracts:`, `protected_files:`) |
| `_reversa_sdd/auto-policy.yaml` | Optional — enables auto mode |
| `_reversa_sdd/audit-policy.json` | Optional — declares fields to redact in the audit log |
| `.reversa/context/graph.json` | The code graph. Built by `reversa graph build`. |
| `.reversa/context/policy-index.json` | Compiled spec frontmatter. Built by `reversa policy-index build`. |
| `.reversa/keeper-queue.jsonl` | Append-only drift queue from the hooks runner |
| `.reversa/audit/YYYY-MM-DD.jsonl` | Audit log |
