...
export default async function migrateReversa(args) {
  ...
  const newSdd = join(projectRoot, 'aegis');
  ...
  try {
    ...
    if (existsSync(oldDir)) {
      if (existsSync(newDir)) {
        ...
        const statePath = join(newDir, 'config', 'state.json');
    ...
    if (existsSync(oldSdd)) {
      if (existsSync(newSdd)) {
        spinner.stop();
        console.log(chalk.yellow('  aegis/ already exists. Skipping specs dir migration.\n'));
        spinner.start('Migrating...');
      } else {
    ...
    if (existsSync(oldSdd)) {
      console.log(`  ${chalk.cyan('Specs:')}     _reversa_sdd/ → aegis/`);
    }
