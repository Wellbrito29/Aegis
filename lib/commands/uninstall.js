...
import { HOOK_GENERATORS } from '../installer/hooks/index.js';
...
export default async function uninstall(args) {
  ...
  // Strip Aegis Spec-installed Keeper hooks before generic file removal.
  // This preserves any unrelated hooks the user added manually.
  for (const gen of HOOK_GENERATORS) {
    try { gen.remove({ projectRoot }); }
    catch { /* best-effort */ }
  }
  ...
  try {
    ...
    console.error(chalk.red('    Error removing aegis/'));
    errors++;
  }
  ...
  console.log('');
  if (errors === 0) {
    console.log(chalk.hex('#ffa203')('  Aegis Spec removed successfully.\n'));
  } else {
