...
const MAPPING = [
  ...
  { from: '_aegis_sdd/changelog', to: 'aegis/changelog', dir: true },
  // Traceability
...
export default async function migrateLayout(args) {
  ...
  // Update state.json output_folder if it points to legacy layout values.
  // For non-default custom folders, we don't auto-rewrite — warn the user
  // so they can decide whether to keep the custom path or align with the
  // single-folder layout.
  const statePath = join(projectRoot, 'aegis', 'config', 'state.json');
  if (existsSync(statePath)) {
    try {
      const state = readJsonSafe(statePath);
      const current = state.output_folder;
      if (current === '_aegis_sdd' || current === '.aegis') {
        state.output_folder = 'aegis';
        writeFileSync(statePath, JSON.stringify(state, null, 2), 'utf8');
      } else if (current && current !== 'aegis') {
        console.log(chalk.yellow(
          `\n  Warning: state.json output_folder is "${current}" (custom value).`
        ));
        console.log(chalk.gray(
          '  Migrated content lives under aegis/, but output_folder still points to your custom path.'
        ));
        console.log(chalk.gray(
          '  Update aegis/config/state.json manually if you want future writes to use aegis/.\n'
        ));
      }
    } catch { /* ignore */ }
  }
