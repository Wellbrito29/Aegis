...
export default async function update(args) {
  ...
  if (!semver.valid(installedVersion)) {
    console.log(chalk.yellow(`  Invalid installed version: "${installedVersion}". Run npx aegis-spec install to fix it.\n`));
    return;
  }

  // Check version on npm
  const spinner = ora({ text: 'Checking for latest version...', color: 'cyan' }).start();
  const latestVersion = await fetchLatestVersion('aegis-spec');
  spinner.stop();
  ...
  try {
    ...
    for (const engine of installedEngines) {
      ...
      if (!hash) continue; // não foi instalado pelo Aegis Spec — não tocar
    ...
    if (latestVersion && semver.valid(latestVersion)) {
      writeFileSync(join(projectRoot, 'aegis', 'config', 'version'), latestVersion, 'utf8');
