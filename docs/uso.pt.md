...
Após instalar, abra o projeto no seu agente de IA e ative o Aegis Spec:
...
É só isso. O Aegis Spec assume o controle e coordena toda a análise a partir daí.

---

## O que acontece quando você ativa

O Aegis Spec verifica se existe uma análise em andamento:
...
**Sessão retomada:** ele lê o checkpoint salvo em `.aegis/state.json` e continua exatamente de onde parou. Não importa se você fechou o editor, reiniciou a máquina ou deixou dormindo por três dias.
...
Você digita /aegis
        ↓
Aegis Spec cria o plano de exploração
        ↓
Você revisa e aprova o plano
        ↓
Scout mapeia a superfície do projeto
        ↓
Aegis Spec apresenta o resumo do Scout e você escolhe o nível de documentação
        ↓
...
Reviewer revisa tudo e levanta perguntas para validação
        ↓
Especificações prontas em _aegis_sdd/
```

O processo é incremental e conversacional. Você não precisa estar presente o tempo todo: o Aegis Spec avisa quando precisa de você.
...
Se a sessão ficar muito longa e o contexto começar a acabar, o Aegis Spec salva o checkpoint automaticamente e avisa:

> "Vou pausar aqui. Tudo está salvo. Digite `/aegis` em uma nova sessão para continuar."
...
Depois que o Scout termina, o Aegis Spec apresenta um resumo do que encontrou (quantos módulos, integrações, se há banco de dados) e pergunta qual volume de documentação você quer para o projeto:
...
A escolha fica salva em `.aegis/state.json` e todos os agentes seguintes a respeitam automaticamente. Se precisar ajustar depois de iniciada a análise, basta editar o campo `doc_level` no arquivo.
