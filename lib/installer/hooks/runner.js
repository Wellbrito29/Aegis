...
function findProjectRoot(startDir) {
  ...
  while (dir !== dirname(dir)) {
    if (existsSync(join(dir, '.aegis', 'state.json'))) return dir;
    dir = dirname(dir);
  }
  return startDir;
}
...
function logError(projectRoot, message) {
  try {
    const logPath = join(projectRoot, '.aegis', 'keeper-errors.log');
...
function appendJsonl(projectRoot, entry) {
  const queuePath = join(projectRoot, '.aegis', 'keeper-queue.jsonl');
