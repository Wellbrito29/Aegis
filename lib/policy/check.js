...
import { AUTO_POLICY_YAML } from '../paths.js';
...
function readAutoPolicyBlacklist(projectRoot) {
  const path = join(projectRoot, AUTO_POLICY_YAML);
  if (!existsSync(path)) return [];
  const out = [];
  // Two-state walk: enter `blacklist:` block (anywhere in indentation),
  // then enter its `paths:` sub-block, then collect dash items.
  let inBlacklist = false;
  let inPaths = false;
  let blacklistIndent = -1;
  for (const rawLine of readFileSync(path, 'utf8').split('\n')) {
    ...
    if (/^blacklist\s*:/.test(trimmed)) {
      inBlacklist = true;
      inPaths = false;
      blacklistIndent = indent;
      continue;
    }
    if (inBlacklist && indent <= blacklistIndent) {
      // exited the blacklist block
      inBlacklist = false;
      inPaths = false;
      // re-process this line in case it's a sibling `blacklist:` (rare)
      if (/^blacklist\s*:/.test(trimmed)) {
        inBlacklist = true;
        blacklistIndent = indent;
      }
    ...
    if (inBlacklist) {
      if (/^paths\s*:/.test(trimmed)) { inPaths = true; continue; }
      // inline list `paths: [a, b]`
      const inline = trimmed.match(/^paths\s*:\s*\[(.+)\]\s*$/);
      if (inline) {
...
export function matchGlob(pattern, file) {
  ...
  let p = pattern
    ...
    .replace(/\/\*\*\//g, STAR)         // mid /**/ — collapse the wrapping slashes
    .replace(/[.+^$|()[\]{}]/g, '\\$&')
    .replace(/\*/g, '[^/]*');
