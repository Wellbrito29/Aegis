// `npx aegis-spec remove-hooks [--engine <id>] [--yes]`
//
// Strips Aegis Spec-installed hooks from engine config files.
// Preserves any other hooks the user added manually (filters by command marker).
// Optionally removes the runner script from aegis/runtime/hooks/.
...
const RUNNER_DEST_REL = join('aegis', 'runtime', 'hooks', 'runner.js');
...
export default async function removeHooks(args) {
  ...
  console.log(chalk.bold('\n  Aegis Spec: Remove Hooks\n'));

  const existing = checkExistingInstallation(projectRoot);
  if (!existing.installed) {
    console.log(chalk.yellow('  Aegis Spec is not installed in this directory.\n'));
    return;
  }
  ...
  if (!opts.yes) {
    const { confirmed } = await inquirer.prompt([{
      ...
      type: 'confirm',
      name: 'confirmed',
      message: `Remove Aegis Spec hooks for ${targets.map((t) => t.name).join(', ')}?`,
      default: true,
    }]);
    if (!confirmed) {
  ...
  for (const gen of targets) {
    ...
    result.cleaned.forEach((p) => {
      console.log(orange(`  ↻  ${relative(projectRoot, p)} (Aegis Spec hooks stripped)`));
      allCleaned.push(p);
    });
    if (result.removed.length === 0 && result.cleaned.length === 0) {
      console.log(chalk.gray(`  ·  No Aegis Spec hooks found for ${gen.name}.`));
    }
  }

  // If no engines have Aegis Spec hooks anymore, drop the runner too.
