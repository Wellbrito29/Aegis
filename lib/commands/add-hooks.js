// `npx aegis-spec add-hooks [--engine <id>] [--yes]`
//
// Installs Keeper hooks for the chosen engine.
// - Copies the runner to aegis/runtime/hooks/runner.js (project-local, no global state)
...
export default async function addHooks(args) {
  ...
  for (const file of plan.files) {
    ...
    if (file.mode != null) {
      try { chmodSync(file.path, file.mode); } catch { /* best-effort on win32 */ }
    }
  ...
  console.log(chalk.gray(`  Remove with: npx aegis-spec remove-hooks --engine ${engineId}\n`));
}
