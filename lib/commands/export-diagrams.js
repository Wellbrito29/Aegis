...
export default async function exportDiagrams(args) {
  ...
  if (!mmdc) {
    ...
    console.log('  Then run: ' + chalk.bold('npx aegis-spec export-diagrams\n'));
    process.exit(1);
  }

  const statePath = join(process.cwd(), 'aegis', 'config', 'state.json');
  ...
  for (const { diagram, source, index } of blocks) {
    ...
    const tmpFile = join(tmpdir(), `aegis-${Date.now()}-${index}.mmd`);
