...
Después de instalar, abre el proyecto en tu agente de IA y activa Aegis Spec:
...
Eso es todo. Aegis Spec toma el control y coordina todo el análisis desde ahí.

---

## Qué ocurre al activarlo

Aegis Spec verifica si hay un análisis en curso:
...
Escribes /aegis
        ↓
Aegis Spec crea el plan de exploración
        ↓
Revisas y apruebas el plan
        ↓
Scout mapea la superficie del proyecto
        ↓
Aegis Spec presenta el resumen del Scout y eliges el nivel de documentación
        ↓
...
Reviewer revisa todo y plantea preguntas de validación
        ↓
Especificaciones listas en _aegis_sdd/
```

El proceso es incremental y conversacional. No necesitas estar presente todo el tiempo: Aegis Spec te avisa cuando te necesita.
...
Si la sesión se alarga y el contexto empieza a agotarse, Aegis Spec guarda el checkpoint automáticamente y avisa:

> "Voy a pausar aquí. Todo está guardado. Escribe `/aegis` en una nueva sesión para continuar."
...
Después de que el Scout termina, Aegis Spec presenta un resumen de lo que encontró (cantidad de módulos, integraciones, presencia de base de datos) y pregunta qué volumen de documentación quieres para el proyecto:
